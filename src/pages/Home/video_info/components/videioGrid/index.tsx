...
import { Context, CustomModalHeader, Grid } from "./styles";
...
import { ButtonClose, NewButton } from "../../styles";
import {
  CustomDropdownItem,
  CustomDropdownMenu,
  ...
  const handleEdit = async () => {
    ...
    try {
      const result = await api.put(
        `/InfoVideos/UpdateVideoDesc?objID=${registro.objID}`,
        registro,
        { headers: header }
      );
      ...
      <Context>
        <Grid>
          {mergedVideos.map((video, index) => {
            ...
            const videoUrl = `https://atekoapi.kingssoftware.com.br/InfoVideos/GetVideoStreaming?InfoVideoId=${videoId}`;

            return (
              <div
                key={index}
                style={{ margin: "10px", width: "640px", marginTop: "-10px" }}
              >
                <video
                  width="100%"
                  height="360"
